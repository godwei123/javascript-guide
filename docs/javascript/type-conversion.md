...
因为没有对应的封装对象，所以 null 和 undefined 不能够被封装,Object(null)和 Object()均返回一个常规对象。NaN 能够被封装为数字封装对象，但拆封之后 NaN == NaN 返回 false，因为 NaN 不等于 NaN。
...
比较运算 ×==y，其中 x 和 y 是值，产生 true 或者 false。这样的比较按如下方式进行：
...
1. 若 Type(x)与 Type(y)相同，则
   ...
   - 若 Type(x) 为 Number，则
     ...
     - 若 x 为-0 且 y 为+0，返回 tue。
     - 返回 false。
   - 若 Type(x) 为 String，则当 x 和 y 为完全相同的字符序列(长度相等且相同字符在相同位置) 时返回 true。否则，返回 false。
   - 若 Type(x) 为 Boolean，当 x 和 y 为同次 true 或者同为 false 时返回 true。否则，返回 false。
