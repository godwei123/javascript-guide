...
## [排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)以及时间复杂度
...
## 项目里面用到了 react-dnd ，它是怎么实现的？和 H5 的拖拽有啥区别？ 说了一下怎么使用的，和 H5 的拖拽使用比较相似，（当时说个人认为是基于
...
## 讲一下原生事件和合成事件的区别 （哭了，说了一个是直接绑定 dom，一个是冒泡到 document 然后统一处理）
...
## 浏览器缓存有哪些？（强缓存和协商缓存），又追问了具体字段有哪些？协商缓存的两对字段分别是什么？Etags
...
## 看你第一个项目用了 webpack，第二个项目用的 vite， 那你说下用过 webpack 或者 vite 的优化配置？（当时说了 webpack
...
## 最简单的[二分查找](https://www.nowcoder.com/jump/super-jump/word?word=二分查找)（什么时候 while 那里不需要写 等号）
